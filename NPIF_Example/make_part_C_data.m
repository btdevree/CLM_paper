...
for SN_index = 1:length(SN_ratios)
    ...
    for param_index = 1:length(sliced_param_array(:));
        ...
        ideal_discrepency_method1 = calculate_discrepency(image, ideal_image, 'normalized_variation_of_information', 100);
        ideal_discrepency_method2 = calculate_discrepency(image, ideal_image, 'sum_of_squares');
        ...
        for pseudorep_index = 1:approx_pseudoreplicates
            ...
            discrepency_1pct_method1 = calculate_discrepency(image_1pct, image_99pct, 'normalized_variation_of_information', 100);
            discrepency_1pct_method2 = calculate_discrepency(image_1pct, image_99pct, 'sum_of_squares');

            % Get the information increase by adding the 1% image to the 99% image
            discrepency_100pct_method1 = calculate_discrepency(image, image_99pct, 'normalized_variation_of_information', 100);
            discrepency_100pct_method2 = calculate_discrepency(image, image_99pct, 'sum_of_squares');
    ...
    % Save CI info
    CI_filename = ['NPIF_part_C_CIdata_SN', num2str(SN_ratio), '.mat'];
