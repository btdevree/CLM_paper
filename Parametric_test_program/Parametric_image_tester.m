...
dots_help_text = {'Put a mark on the center of the dozen or less small, bright regions'; 'Left click = make new center mark'; 'Left click & drag = move center mark'; 'Right click = remove center mark'; 'Center button = toggle zoom mode'};
lines_help_text = {'Draw a curve that follows the half-dozen or less lines or curves in the image.'; 'Lift click and drag on empty space to set out a new curve.'; 'If needed, left click and drag center point to line up highlighted curve.'; 'Right click = remove highlight curve'; 'Center button = toggle zoom mode'};
...
set(hscatter, 'LineWidth', 1, 'SizeData', 60);
...
function next_button_callback(source, callbackdata, skip_save_flag)
    ...
    % Set text headings and display help dialogs if this is the first time we've seen this image type
    ...
    if strcmp(current_image_type, 'region')
        ...
        if show_region_dialog
            waitfor(msgbox(region_help_text, 'New Image Type'));
    ...
    elseif strcmp(current_image_type, 'dots')
        ...
        if show_dots_dialog
            waitfor(msgbox(dots_help_text, 'New Image Type'));
            show_dots_dialog = false;
        end
    elseif strcmp(current_image_type, 'actin')
        ...
        if show_lines_dialog
            waitfor(msgbox(lines_help_text, 'New Image Type'))
            show_lines_dialog = false;
        end
    elseif strcmp(current_image_type, 'border')
        ...
        if show_border_dialog
            waitfor(msgbox(border_help_text, 'New Image Type'));
...
function [X, Y, C] = get_point_data
    ...
    if strcmp(current_image_type, 'region')
        ...
        % Orange for the first point, Red for all others
        X = current_answer_x(:);
        Y = current_answer_y(:);
    ...
    elseif strcmp(current_image_type, 'dots')
        ...
        % Orange for the last, Red for all others
        X = current_answer_x(:);
        Y = current_answer_y(:);
        C = repmat([1, 0, 0], size(X, 1), 1);
        if size(X, 1) > 0
    ...
    elseif strcmp(current_image_type, 'actin')
        ...
        % Only plot control points for current line
        [num_lines, num_cp] = size(current_answer_x);
        X_current = current_answer_x(num_lines, :);
        Y_current = current_answer_y(num_lines, :);
        X = X_current(:);
        Y = Y_current(:);
        C = repmat([1, 0, 0], size(X, 1), 1);
...
function [X, Y, C, P] = get_line_data
    ...
    elseif strcmp(current_image_type, 'actin')
        ...
        % Get coordinates for control net lines
        if size(current_answer_x, 2) == 2
            X_mat = current_answer_x(end, :)';
            Y_mat = current_answer_y(end, :)';
        elseif size(current_answer_x, 2) == 3
            X_mat = horzcat(current_answer_x(end, 1:2)', current_answer_x(end, 2:3)');
            Y_mat = horzcat(current_answer_y(end, 1:2)', current_answer_y(end, 2:3)');
        elseif size(current_answer_x, 2) == 4
            X_mat = horzcat(current_answer_x(end, 1:2)', current_answer_x(end, 2:3)', current_answer_x(end, 3:4)');
            Y_mat = horzcat(current_answer_y(end, 1:2)', current_answer_y(end, 2:3)', current_answer_y(end, 3:4)');
        else % no answer yet
            X_mat = [];
            Y_mat = [];
        end
        ...
        % Red for current control net line, control net not pickable
        C_cn = repmat([1, 0, 0], size(X_mat, 2), 1);
