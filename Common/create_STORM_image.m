function sparse_image = create_STORM_image(data, resolution, sigma, dims, calc_cutoff_sigmas, zero_cutoff)
    ...
    % Set defaults
    if nargin < 6; zero_cutoff = 1e-6; end 
    if nargin < 5, 
        calc_cutoff_sigmas = 5; 
    end
    ...
    for data_ind = 1:length(data.x)
        ...
        % Add to total
        dense_image(min_row:max_row, min_column:max_column) =...
            dense_image(min_row:max_row, min_column:max_column) + reshape(pdf, size(x_mesh));
    end
